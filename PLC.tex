...
Intel8080.tokenising.data.numeric = {
    ...
    hex = lpeg.Ct(lpeg.Cg(lpeg.R "09"
        * (lpeg.R "09" + lpeg.R "af" + lpeg.R "AF")^0 / string.upper, "value")
...
-- Intel8080.formatting.make_listing = function (state)
--     local listing = ""
-- 
--     for line_no, line in ipairs(state) do
--         local print_line = string.format("%#x    ", line.location_counter)
-- 
--         if line.label then
        ...
        context.startxrow()
            context.startxcell()
                ...
                for i, _ in ipairs(line.bytes) do
                    if i == 1 then
                        result = string.format([==[\tech{%.4XH}]==], counter)
                    else
                        result = result .. string.format([==[\blank[none]\tech{%.4XH}]==], counter)
                    end
                    counter = counter + 1
                end
                context(result)
            ...
            context.startxcell({align = "center, lohi", width = "\\dimexpr\\widthofstring{Машинный} + 1em"})
                local result
            ...
            context.startxcell({align = "flushleft", width = "\\dimexpr\\widthofstring{Ассемблерный код} + 5em"})
                local args = ""
                if line.asm.args then
                    ...
                    context([==[\tech{\tt %s}]==], line.asm.cmd_name)
                end
            context.stopxcell()

            context.startxcell({align = "width, stretch, morehyphenated"})
